...
export default function Test() {
  ...
  const finalizeCheckIn = async (
    ...
    const result = await testDataService.insertCheckIn({
      venue: checkIn.venue,
    ...
    if (normalizedAdjusted.length > 0) {
      try {
        await testDataService.updateMultipleCheckIns(
        ...
        console.error(
          "Failed to update conflicting check-ins in test data:",
          error
        );
        throw error;
      }
  ...
  // Function to load check-ins from test data
  const loadCheckIns = async () => {
    try {
      const testData = await testDataService.fetchCheckIns();

      const convertedCheckIns: CheckIn[] = testData.map(
        (testCheckIn: SupabaseCheckIn) => {
          const startFallbackTime =
            extractTimeFromTimestamp(
              testCheckIn.start_time ?? testCheckIn.created_at
            ) || DEFAULT_START_TIME;

          const normalizedStart = normalizeDateTime({
            raw: testCheckIn.start_time,
            date: testCheckIn.date,
            fallbackTime: startFallbackTime,
          });

          const normalizedEnd = normalizeDateTime({
            raw: testCheckIn.end_time,
          ...
          const venue = OHIO_STATE_VENUES.find(
            (v) => v.name === testCheckIn.venue
          );

          return {
            id: testCheckIn.id,
            venue: testCheckIn.venue,
            ...
            startDateTime,
            endDateTime,
            timestamp: new Date(testCheckIn.created_at),
          };
        }
      );
      ...
      console.error("Error loading check-ins from test data:", error);
    }
  };

  // Fetch check-ins from test data on page load
  useEffect(() => {
    loadCheckIns();
  }, []);
  ...
  const handleDeleteCheckIn = async (id: string) => {
    try {
      await testDataService.deleteCheckIn(id);
